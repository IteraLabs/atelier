...
fn main() {
    ...
    for i in 0..=n_progressions {
        ...
        let i_ret_gbm_bids: f64 = brownian::gbm_return(
            i_bid_price, mu, sigma, 0.1, 1).unwrap()[0];
        let i_ret_gbm_asks: f64 = brownian::gbm_return(
            i_ask_price, mu, sigma, 0.1, 1).unwrap()[0];
    ...
    for i in 0..=n_progressions {
        ...
        // Compute the VWAP
        v_vwap.push(market::VWAP::compute(&iter_bids.clone(), &iter_asks.clone(), 4));
