...
fn main() {
    ...
    // Load configuration
    let config = configs::Config::load_from_toml(config_file).unwrap().clone();
    ...
    for i_e in 0..9 {
        ...
        let bid_levels = exchanges[i_e].orderbook.clone().unwrap().bid_levels.unwrap();
        let bid_orders = exchanges[i_e].orderbook.clone().unwrap().bid_orders.unwrap();
        ...
        let ask_levels = exchanges[i_e].orderbook.clone().unwrap().ask_levels.unwrap();
        let ask_orders = exchanges[i_e].orderbook.clone().unwrap().ask_orders.unwrap();
        ...
        println!("\nLEVEL STATS: ");
        println!("\n  No. of Bids levels stats (min, max, mean) {:?}", stats_bids);
        println!("  No. of Asks levels stats (min, max, mean) {:?}\n", stats_asks);
