...
mod test_orderbook_utils {
    ...
    use atelier::data::market::{Level, Orderbook, Side};
...
mod tests {
    ...
    fn find_level_output_value() {
        ...
        println!(
            "randomly picked Level.level_id: {:?}",
            random_level.level_id
        );
        ...
        if level_found.as_ref().unwrap() > &1 {
            ...
            println!(
                "Ask level price: {:?}",
                testable_ob.asks[a_level as usize].price
            );
            ...
            println!(
                "Bid level price: {:?}",
                testable_ob.bids[b_level as usize].price
            );
